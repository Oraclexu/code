...
from models import Generator, Discriminator,DiscriminatorS, weights_init_normal
...
class Solver:
    ...
    def train(self):
        ...
        fff_f = torch.cat([fixed_S,fixed_S,fixed_S,fixed_S,fixed_S],dim=1)
        onesS = torch.ones_like(self.discriminator(fff_f, fixed_S))
        zerosS = torch.zeros_like(self.discriminator(fff_f, fixed_S))
        ...
        for epoch in range(self.start_epoch, 202):
            ...
            for iteration, data in enumerate(self.loaders.train_loader):
                ...
                ####################
                iqaloss = contentFunc(fake_images,source_images)
                # fff = torch.cat([fake_S,fake_S,fake_S,fake_S,fake_S],dim=1)          
                # ganS_loss = self.criterion_GAN(self.discriminator(fff, fake_S), onesS)
                ##################
                recon_loss = self.criterion_recon(fake_images, target_images)
                reconS_loss = self.criterion_recon(fake_S, target_S)

                g_loss = gan_loss + 100 * recon_loss + 100 * reconS_loss + iqaloss#+ ganS_loss
                ...
                # fff_target=torch.cat([target_S, target_S, target_S, target_S, target_S], dim=1)
                # fff_fake=torch.cat([fake_S,fake_S,fake_S,fake_S,fake_S],dim=1)
                # loss_real = self.criterion_GAN(self.discriminator(target_S, fff_target), onesS)
                # loss_fake = self.criterion_GAN(self.discriminator(fake_S.detach(), fff_fake.detach()), zerosS)
                ...
                print('[EPOCH:{}/{}]  [ITER:{}/{}]  [D_GAN:{}]  [G_GAN:{}]  [RECON:{} [RECONS:{}][iqa:{}]'.
                      format(epoch, 200, iteration, len(self.loaders.train_loader),
                       d_loss, gan_loss, recon_loss,reconS_loss,iqaloss))
    ...
    def test(self):
        ...
        # Save Images for debugging
        self.save_images_path = '/home/l/my/dataset/pix2pix/ssim_1d/'
        self.generator = self.generator.eval()
        ...
        with torch.no_grad():
            for i, (source_images,A_name) in enumerate(self.loaders.test_loader):
                ...
                save_image((fake_images.cpu()+1.0)/2.0,
                           os.path.join(self.save_images_path, 'test/{}_B.png'.format(A_name)))
                save_image((source_images.cpu()+1.0)/2.0,
                           os.path.join(self.save_images_path, 'test/{}_A.png'.format(A_name)))
                save_image((fake_S.cpu() + 1.0) / 2.0,
                           os.path.join(self.save_images_path, 'test/{}_S.png'.format(A_name)))
